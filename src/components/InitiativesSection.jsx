...
const stats = [
  {
    ...
    animation: "stat-left-animate",
  },
  {
    number: "10000+",
    label: "Students Educated",
    animation: "stat-right-animate",
  },
  {
    number: "25+",
    ...
    <section ref={sectionRef} className="w-full bg-white py-12 sm:py-16">
      <div className=" mx-auto">
        <div className="bg-[#2D2D2D] rounded-[32px] sm:rounded-[40px] px-5 sm:px-8 md:px-12 lg:px-16 py-12 sm:py-14 md:py-16 overflow-hidden">
          ...
          {/* Cards */}
          <div className="max-w-[1000px] mx-auto py-5 grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5">
            {initiatives.map((item, index) => (
              <div
                key={index}
                className={`bg-white rounded-[18px] sm:rounded-[20px] p-4 sm:p-5 flex items-center gap-4 ${
              ...
              >
                ...
                <div>
                  <h2 className="font-heading font-extrabold text-black text-[18px] sm:text-[20px] leading-tight">
                    {item.title}
                  </h2>

                  <p className="mt-2 text-black text-[15px] sm:text-[15px] leading-[17px]">
                    {item.description}
                  </p>

                  <a
                    href="#"
                    className="group relative inline-flex mt-3 text-[11px] sm:text-[12px] font-medium text-black overflow-hidden"
                  >
                    <span>Learn More</span>
                    <span className="absolute left-0 bottom-0 w-full h-[2px] bg-[#F5BC18]"></span>
                    <span className="absolute left-0 bottom-0 w-full h-[2px] bg-[#67E8F2] translate-x-[-100%] group-hover:translate-x-0 transition-transform duration-300"></span>
                  </a>
                </div>
              </div>
            ))}
          ...
          {/* Stats */}
          <div className="mt-12 sm:mt-16 md:mt-20 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
            {stats.map((stat, index) => (
              <div
                key={index}
                className={`relative min-h-[130px] sm:min-h-[180px] lg:min-h-[360px] px-4 sm:px-7 flex ${
                  index === 0
              ...
              >
                <div
                  className={`${
                    startAnimation
                      ? `${stat.animation} delay-${(index + 1) * 100}`
                      : "opacity-0"
                  }`}
                >
