...
// Change paths according to your folder
...
const Navbar = () => {
    ...
    // Stops page scrolling when menu is open
    useEffect(() => {
    ...
    return (
        <>
            {/* MAIN NAVBAR */}
            <header className="w-full bg-white border-t-[4px] border-[#20343b] relative z-40">
                <nav className="h-[90px] sm:h-[110px] md:h-[125px] flex items-center justify-between px-6 sm:px-12 md:px-20 lg:px-24">
                    {/* Logo */}
            ...
            {/* MENU OVERLAY */}
            <div
                className={`fixed top-0 left-0 w-full h-screen bg-[#2D2D2D] border-t-[4px] border-[#20343b] z-[9998] overflow-hidden transform transition-transform duration-[850ms] ease-in-out ${isOpen ? "translate-y-0" : "-translate-y-full"
                    }`}
            >
                {/* Close Button */}
                <button
                            ...
                            {navLinks.map((link, index) => (
                                <li key={link.name}>
                                    <a
                                        href={link.path}
                                        onClick={() => setIsOpen(false)}
                                        className={`block font-heading font-extrabold text-[34px] sm:text-[40px] md:text-[42px] lg:text-[44px] leading-none tracking-[-1px] transition-colors duration-300 ${index === 0
                                                ? "text-[#F5BC18]"
